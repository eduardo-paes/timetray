...
export function DashboardLayout() {
  ...
  return (
    <div className="flex h-screen bg-obs-950 text-steel-primary overflow-hidden">
      {/* Left sidebar */}
      <aside className="w-64 border-r border-obs-500/60 flex flex-col flex-shrink-0">
        <SidebarLeft
          tasks={store.tasks}
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        {/* Header */}
        <header className="flex items-center justify-between px-8 py-5 border-b border-obs-500/60 flex-shrink-0">
          <div>
            <div className="text-[10px] text-steel-disabled uppercase tracking-widest mb-1">
              Now Tracking
            </div>
            {activeTask ? (
              <div
                className="text-lg font-semibold"
                style={{ color: activeTask.color ?? '#C87A2B' }}
              >
                {activeTask.name}
              </div>
            ) : (
              <div className="text-lg font-semibold text-steel-disabled">Nothing tracked</div>
            )}
          </div>
          ...
          <div className="flex items-center gap-4">
            {store.activeSession && (
              <>
                <div className="text-5xl font-mono font-bold tabular-nums text-steel-primary tracking-tight">
                  {formatDuration(elapsed)}
                </div>
                <button
                  onClick={handleStopTracking}
                  className="px-3 py-1.5 text-xs border border-obs-500 text-steel-muted hover:text-steel-primary hover:border-copper rounded-[10px] transition-colors"
                >
                  Stop
                </button>
              </>
        ...
        {/* Task switch buttons */}
        <div className="px-8 py-4 flex gap-2 flex-wrap border-b border-obs-500/40 flex-shrink-0">
          {store.tasks
            .filter((t) => t.enabled)
            .map((task) => {
              ...
              return (
                <button
                  ...
                  ${
                    isActive
                      ? 'bg-copper/10 border border-copper/50 text-copper-bright cursor-default'
                      : 'bg-obs-800 border border-obs-500 text-steel-secondary hover:bg-obs-700 hover:border-obs-500 cursor-pointer'
                  }
                `}
                >
                  <span
                  ...
                  />
                  {task.name}
                  {isActive && <span className="text-[10px] text-copper animate-pulse">▶</span>}
                </button>
              )
            })}
        </div>
      ...
      {/* Right sidebar */}
      <aside className="w-64 border-l border-obs-500/60 flex flex-col flex-shrink-0">
        <SidebarRight
      ...
      {editingTask && (
        <div
          className="fixed inset-0 bg-obs-950/80 flex items-center justify-center z-50 p-4"
